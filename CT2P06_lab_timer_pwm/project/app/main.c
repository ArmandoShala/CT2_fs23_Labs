...
int main(void) {
    ...
    // Initialisiert den Timer TIM3
    tim3_init();

    // Initialisiert den Timer TIM4
    tim4_init();

    // Setzt PWM-Werte für Kanäle 1, 2 und 3
    // tim3_set_compare_register(CHANNEL1, 2048);
    // tim3_set_compare_register(CHANNEL2, 1024);
    // tim3_set_compare_register(CHANNEL3, 512);

    // Endlose Schleife
    while (1) {
        // Warten auf den Interrupt
    }
...
void TIM4_IRQHandler(void) {
    ...
    /* Toggle LEDs */
    if (cycle_counter_4bit % 2 == 0) {
        /* Write word of data to all LEDs. */
        CT_LED->WORD = (uint32_t) 0xFF;
    } else {
        /* Write word of data to all LEDs. */
        CT_LED->WORD = (uint32_t) 0x00;
    }
