...
void tim3_init(void) {
  ...
  TIM3->CR1 = 0x0000; // setup upcounter

  // 84'000'000 / (x * 2^16) = 200
  // x ~ 6.4...
  TIM3->PSC = 7 - 1;
  // x = 84'000'000 / (7 * 200) = 60'000
  TIM3->ARR = 60000 - 1;

  // set pwm mode for all three channels
  TIM3->CCMR1 |= (0b110 << 4) | (0b110 << 12);
  TIM3->CCMR2 |= (0b110 << 4);

  // enable output for all three channels
  TIM3->CCER = (1 << 0) | (1 << 4) | (1 << 8);
...
void tim4_init(void) {
  ...
  TIM4->PSC = 8400 - 1;
  TIM4->ARR = 10000 - 1;
  TIM4->CR1 = 0x0010;      // setup downcounter
  TIM4->DIER = 0x00000001; // enable interrupt on overflow
...
void tim3_set_compare_register(pwm_channel_t channel, uint16_t value) {
  ...
  // volatile uint32_t *addr = &TIM3->CCR1 + (sizeof(uint32_t) * channel);
  // *addr = value;
